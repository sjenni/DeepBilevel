...
from train.StandardTrainer import StandardTrainer
...
def train_test_model(noise):
    ...
    tag = 'standard_noiselevel_{}'.format(noise)

    # Initialize the data generator
    label_file = os.path.join(CIFAR10_DATADIR, 'cifar10_train_noisy_{}.txt'.format(noise))
    data_gen_train_noisy = CIFAR10Generator(label_file, batch_size, batch_splits=1)

    # Define the network and training
    model_noisy = InceptionCifar(batch_size=batch_size, im_shape=im_shape, tag=tag)
    trainer = StandardTrainer(model=model_noisy, data_generator=data_gen_train_noisy, pre_processor=preprocessor,
